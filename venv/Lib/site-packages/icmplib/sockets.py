...
class ICMPSocket:
    ...
    '''
    def __init__(self, config, src_addr=None):
        self._config = config
        self._last_request = None

        try:
            self._socket = config.IP_SOCKET(
                config.IP_PROTOCOL, src_addr)
    ...
    def _read_reply(self, packet, source, reply_time):
        ...
        icmp_type, icmp_code = unpack('!BB', packet[
            self._config.ICMP_HEADER_OFFSET:
            self._config.ICMP_CHECKSUM_OFFSET])

        if icmp_type != self._config.ICMP_ECHO_REPLY:
            icmp_id = None
            icmp_sequence = None
        else:
            icmp_id, icmp_sequence = unpack('!HH', packet[
...
class ICMPv4Socket(ICMPSocket):
    ...
    '''
    def __init__(self, src_addr=None):
        super().__init__(ICMPv4Config, src_addr)
...
class ICMPv6Socket(ICMPSocket):
    ...
    '''
    def __init__(self, src_addr=None):
        super().__init__(ICMPv6Config, src_addr)
